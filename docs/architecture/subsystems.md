...
browser. Three transports cascade from lowest-latency to most-
compatible — WebRTC (P2P SRTP, <100 ms), H.264 / MSE (~300 ms),
JPEG / canvas (~500 ms). Phase 6a (#134) added a concurrency cap
(`PREVIEW_MAX_ACTIVE`), per-tab heartbeat with auto-restart, a
dedicated `preview` audit category, and a "Previews" monitoring
section.

**Source:** `src/lib/preview-stream/`

**Deep dive:** [`docs/architecture/preview-stream.md`](./preview-stream.md)
covers the connect sequence, frame / input flow, wire protocol, error
codes, resource quotas, and full file inventory.

**Operations:** [`docs/operations/preview-tuning.md`](../operations/preview-tuning.md)
covers tuning knobs, log locations, the SLO table, and a
common-failures recovery playbook.
