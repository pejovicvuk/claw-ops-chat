...
async function postHandler(request: Request): Promise<Response> {
  ...
  let body: { clientId?: unknown; clientSecret?: unknown; tenantId?: unknown };
  try {
  ...
  // re-entering credentials (e.g. rotating the client secret) does not
  // invalidate an active session. If the clientId has actually changed,
  // the next status refresh will detect an invalid_grant and surface
  // tokenExpired=true, prompting the user to re-authorize.
  ...
  try {
    ...
    // the env block stays up-to-date with any new clientSecret.
