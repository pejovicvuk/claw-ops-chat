...
import { deleteRule, getRule, updateRule } from "@/lib/monitoring/alerts/store";
...
export async function PATCH(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await getAuditWriter()
    .alert({
      ...
      severity: "info",
      actor: session.email,
      subject: `Alert rule updated: ${updated.name}`,
      durationMs: null,
      ruleId: updated.id,
...
export async function DELETE(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await getAuditWriter()
    .alert({
      ...
      severity: "info",
      actor: session.email,
      subject: `Alert rule deleted: ${existing?.name ?? id}`,
      durationMs: null,
      ruleId: id,
