...
export const FileBrowser = forwardRef<FileBrowserHandle, FileBrowserProps>(
  function FileBrowser({ onFileClick, onFileOpen, onCopyPath }, ref) {
        ...
        {contextMenu && (
          ...
          >
            {!contextMenu.entry.directory && (
              <button
                type="button"
                onClick={() => { onFileOpen?.(contextMenu.entry); closeMenu(); }}
            ...
            {!contextMenu.entry.directory && (
              <button
                type="button"
                onClick={() => { downloadFile(contextMenu.entry.path); closeMenu(); }}
                className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-[12px] text-canvas-fg hover:bg-canvas-surface-hover"
              >
                <FiDownload size={12} />
                Download
