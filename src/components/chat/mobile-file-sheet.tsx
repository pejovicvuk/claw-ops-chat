...
export function MobileFileSheet({
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-black/30 backdrop-blur-[2px] ${exiting ? "animate-backdrop-out" : "animate-backdrop-in"}`}
        style={{ zIndex: Z_INDEX.MODAL }}
        onClick={onClose}
      />
      <div
        className={`fixed inset-x-0 bottom-0 flex flex-col rounded-t-2xl border-t border-canvas-border bg-canvas-bg shadow-xl ${exiting ? "animate-sheet-down" : "animate-sheet-up"}`}
