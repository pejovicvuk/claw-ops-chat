...
async function resolvePowerShell() {
    if (detectPlatform() !== "win32")
        return null;
    if (psResolved !== undefined)
        return psResolved;
    const candidates = [
        "powershell.exe",
        "pwsh.exe",
        process.env.WINDIR
            ? (0, path_1.join)(process.env.WINDIR, "System32", "WindowsPowerShell", "v1.0", "powershell.exe")
            : null,
    ].filter((c) => !!c);
...
async function buildUvInstallCommand() {
    ...
    if (platform === "win32") {
        ...
        if (!ps) {
            return {
                error: "No PowerShell found on this server. Install PowerShell 7 (pwsh) or run the install command manually.",
            };
        }
